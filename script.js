...
function getFromURL(files) {
    ...
    reader.onload = function (e) {
        const contents = e.target.result.match(/<body[\s\S]*<\/body>/i);
        if (contents.length !== 1) {
            alert("Retrieved file did not have a body");
        }
        const el = document.getElementById('webContent');
        el.innerHTML = contents[0];
        $rw_tagSentences();
        reader.onerror = function (e) {
            alert("Unable to read file: " + JSON.stringify(e));
        };
    }
